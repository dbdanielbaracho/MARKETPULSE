...
| MP-045 | PrediBeacon Pro subscriptions for advanced intelligence | IN_PROGRESS | hosted Stripe Checkout/Customer Portal and webhook-signature adapter already implemented fail-closed; configurable opaque product identity plus durable SQLite WAL subscription-state projection now provide fail-closed product/status/expiry feature authorization with adversarial tests; authenticated account/customer binding, verified provider-event projection into entitlements, live credentials and production lifecycle evidence remain pending |
