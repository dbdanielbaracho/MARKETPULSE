...
2. user-visible reported USD values are formatted with the selected document locale through `Intl.NumberFormat`; PrediBeacon does not fabricate FX conversion or silently relabel venue-reported currency;
3. relative time uses `Intl.RelativeTimeFormat` and absolute market timestamps use `Intl.DateTimeFormat` in the browser's local timezone with a timezone abbreviation;
...
- `tests/test_locale_number_time.py`
