...
@pytest.mark.asyncio
async def test_diag_kalshi_politics_tech_inventory() -> None:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        # Resolve series metadata for every series in the 5k pool so category
        # counts are based on Kalshi's own category when available.
    ...
    for provider_rank, x in enumerate(ranked, 1):
        ...
        if cat in {"politics", "technology"} or str(provider_cat).lower() in {"politics", "technology", "tech"}:
            out.append({
                "rank": provider_rank,
                "category": cat,
                "provider_category": provider_cat,
                "title": title,
                "ticker": x.get("ticker"),
                "volume_24h_usd": KalshiAdapter._usd_notional_volume_24h(x),
                "volume_usd": KalshiAdapter._usd_notional_volume(x),
    ...
    # Deliberate failure keeps this diagnostic branch out of production and makes logs easy to retrieve.
