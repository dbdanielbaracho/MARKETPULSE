...
def test_market_journey_is_localized_without_script_injection():
    ...
    out = localize_public_html('/market', source, 'pt-BR')
...
def test_market_slug_route_uses_selected_locale():
    source = '<html lang="en"><body>← Back to markets Probability history</body></html>'
    out = localize_public_html('/markets/example-market', source, 'pt-BR')
...
def test_alert_journey_localizes_static_and_dynamic_states():
    ...
    out = localize_public_html('/alerts', source, 'pt-BR')

    for phrase in ('← Mercados', 'ACOMPANHE O QUE IMPORTA', 'Alertas de mercado', 'Receba uma notificação no navegador', 'Criar um alerta', 'Alertas salvos', 'Escolha um mercado primeiro', 'Alerta salvo para', 'Probabilidade atual indisponível'):
        assert phrase in out


def test_english_is_default_locale():
    source = '<html lang="en"><body>← Markets Market alerts Probability history</body></html>'
    out = localize_public_html('/alerts', source)
    assert '<html lang="en">' in out
    assert '← Markets' in out
    assert 'Market alerts' in out
    assert 'Probability history' in out


def test_unrelated_html_is_not_modified():
    source = '<html lang="en"><body>Market alerts Probability history</body></html>'
    assert localize_public_html('/admin', source, 'pt-BR') == source
