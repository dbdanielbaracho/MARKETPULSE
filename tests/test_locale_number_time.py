...
def test_public_market_html_uses_selected_document_language_for_intl_formatting():
    response = client.get('/market?market_id=kalshi:test', headers={'Accept-Language': 'pt-BR,pt;q=0.9'})
    assert response.status_code == 200
    assert response.headers['content-language'] == 'pt-BR'
    assert 'document.documentElement.lang' in response.text
    assert 'Intl.RelativeTimeFormat' in response.text
    assert "Intl.NumberFormat('en-US'" not in response.text
