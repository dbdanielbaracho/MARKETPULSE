...
def test_relevance_outranks_forced_provider_balance():
    now = datetime(2026, 8, 22, tzinfo=timezone.utc)
    set_discovery_markets([
        DiscoveryMarket(canonical_id=f"polymarket:high-{index}", title=f"Poly {index}", venue="polymarket", volume_usd=100, trend_score=100-index, observed_at=now)
        for index in range(5)
    ] + [
        DiscoveryMarket(canonical_id=f"kalshi:lower-{index}", title=f"Kalshi {index}", venue="kalshi", volume_usd=100, trend_score=50-index, observed_at=now)
...
def test_near_relevance_scores_receive_soft_provider_balance():
    now = datetime(2026, 8, 22, tzinfo=timezone.utc)
    set_discovery_markets([
        DiscoveryMarket(canonical_id=f"polymarket:near-{index}", title=f"Poly near {index}", venue="polymarket", volume_usd=100, trend_score=100-index, observed_at=now)
        for index in range(5)
    ] + [
        DiscoveryMarket(canonical_id="kalshi:near", title="Kalshi near", venue="kalshi", volume_usd=100, trend_score=92, observed_at=now)
    ])
    data = client.get("/api/v1/markets?sort=trending&limit=4").json()
